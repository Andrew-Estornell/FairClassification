...
class RegOracle:
    ...
    def predict(self, X):
        p0 = self.b0.predict(X)
        p1 = self.b1.predict(X)
        y = np.greater(p0, p1).astype(int)
        #print('1', y)
        return y
...
class RandomLinearThresh:
    ...
    def predict(self, X):
        y = np.multiply(X, np.reshape(self.coefficient, [1, len(self.coefficient)]))
        y = y < 0
        print('2', y.astype(int))
        return y.astype(int)
