...
import incentive_compatable.audting2 as ic_auditing
		...
		for alpha in alphas:
			...
			for split in d:
				...
				opt_lies_all_clfs, opt_pred_ps_all_clfs, opt_preds_all_clfs =\
					manip.optimal_agent_strats_for_cata_features(X, y, manip_cols, clfs.values(), alpha=alpha, decision_type='preds')


				###############################################
				# IC attempt
				prob_ic = np.full(( 1, len(manip_cols)), 1/len(manip_cols))
				# prob_ic = np.random.uniform(0,1,(len(clfs), len(manip_cols)))

				maxiterationIC = 40#10000

				lie_count_dict = {
					'total': 0
				}
				for i in range(len(manip_cols)):
					lie_count_dict[i] = 0

				gain_array_history = []

				for i in range(maxiterationIC):
					# for clf in clfs
					ic_lies, ic_pred_prob_all_clfs, ic_preds_all_clfs, manip_col_indexes = ic_auditing.ic_strats(X, y,  manip_cols, clfs.values(), prob_ic, alpha, decision_type='preds')
					prob_ic, prob_ic_allclfs, lie_count_dict = ic_auditing.update_ic_prob(X, ic_lies, clfs.items(), manip_col_indexes, lie_count_dict, gain_array_history) # I think the individual clfs can be dealt later, so sent all X and  ic_lies for all clfs at once
					print("current total lies: ", lie_count_dict['current'])
					print("total lies: ", lie_count_dict['total'])
					print('total gain: ', sum(gain_array_history[-1]))
					# print(sum(gain_array_history[-1]))
